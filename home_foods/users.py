...
@app.post("/sign-up")
async def sign_up_user(email: str, password: str, firstName: str, lastName: str):
    data = users(email, password, firstName, lastName)
    sign_up_response = data.sign_up()
    return sign_up_response


@app.get("/login")
def login_response(email: str, password: str):
    data = users.login(email, password)
    return data


@app.get("/send-rest-passcode")
async def send_reset_passcode(email: str):
    data = users.send_store_passcode(email)
    return data


@app.post("/update-password")
async def update_password(email, otp, newPassword):
    data = users.update_forgot_password(email, otp, newPassword)
    return data


@app.post("/reset-password")
async def reset_password(email, currentPassword, newPassword):
    data = users.reset_password(email, currentPassword, newPassword)
    return data
